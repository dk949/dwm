...
backlight_error_t bright_get_impl(FILE *fp, int *oldValue) {
    // TODO(dk949): Just use std::from_chars...
    char buf[5];
    ...
    *oldValue = -1;
    switch (bytesRead) {
        ...
        default: {
            lg::error("backlight: Internal error: read a weird number of bytes from brightness file: {}", bytesRead);
            return BACKLIGHT_INTERNAL_ERROR;
        }
    }
    if (*oldValue == -1) {
        lg::error("backlight: Internal error: something went very wrong when decoding brightness file");
        return BACKLIGHT_INTERNAL_ERROR;
    }
    return BACKLIGHT_OK;
}
...
backlight_error_t bright_modify(double value, int dir) {
    ...
    if (!fp) {
        lg::error("backlight: Could not open brightness file for  modification: {}", std::strerror(errno));
...
backlight_error_t bright_get_(double *value) {
    FILE *fp = fopen(filename, "r");
    if (!fp) {
        lg::error("backlight: Could not open brightness file for reading: {}", std::strerror(errno));
        return BACKLIGHT_OPEN_ERROR;
    }
    int oldValue;
    backlight_error_t ret = bright_get_impl(fp, &oldValue);
