...
template<typename... Args>
struct FnPtrVariant<std::variant<Args...>> {
    template<typename T>
    using GetFnType = std::conditional_t<std::is_same_v<T, std::monostate>,
        void (*)(),
        std::conditional_t<(sizeof(T) > 2 * sizeof(void *)), void (*)(T const &), void (*)(T)>>;
    using type = std::variant<GetFnType<Args>...>;
};

template<Variant T>
using FnPtrVariantT = FnPtrVariant<T>::type;
...
static_assert(std::is_same_v<  //
    ...
    FnPtrVariantT<std::variant<std::monostate, int, float, std::string>>  //
    >);

template<Variant V>
constexpr bool variantInvoke(FnPtrVariantT<V> const &fn, V const &arg) {
